// 한국어 성경 약어 → 책 이름 전체 매핑.
// 웨스트민스터 요리문답/신앙고백서 references와 krv.json 에서 공통으로 사용하는 약어 기준(개역한글 66권).
...
/** 약어(또는 전체 이름)로 책 전체 이름을 반환. 모르면 입력 그대로 반환. */
export function bookFullName(abbr: string): string {
  return bibleBookMap[abbr] ?? abbr;
}
