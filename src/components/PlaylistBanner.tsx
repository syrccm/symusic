...
// - 시안 C 2줄: 윗줄 [CLICK 배지] / 아랫줄 「{text}」 새창으로 듣기 — text 는 config/banner.text 그대로, 표시할 때만 「」·"새창으로 듣기" 가공
//   · 배지: 분홍(#ED93B1) 알약, 안에 커서 아이콘 + "CLICK"(12px, 어두운 분홍 #4B1528 글자)
//     분홍↔청록(#3FCDB8) 깜빡임은 index.css 의 .animate-badge-blink(동작 줄이기 설정 시 정지)
//   · 텍스트: 분홍빛 흰색(#f0dced) 14px, 배경 없음·왼쪽 정렬, 잘리지 않게 전부 표시(break-keep)
...
export function PlaylistBanner({ text, onClick }: PlaylistBannerProps) {
  ...
  return (
    <button
      ...
      onClick={onClick}
      aria-label={label}
      className="group w-full rounded-lg pl-[9px] pr-3 py-1.5 flex flex-col items-start gap-1
    ...
    >
      ...
      {/* 아랫줄: 「제목」 새창으로 듣기 — 말줄임 없이 전부 표시(길면 단어 단위 줄바꿈) */}
      <span className="w-full text-left text-sm font-semibold leading-snug break-keep text-[#f0dced] group-hover:text-white transition-colors">
        {label}
      </span>
    </button>
  );
