// daylong 화면 공통 표시 유틸 — 원화·날짜 포맷, 최근 N개월 열, Firestore 오류 문구.
...
export function recentMonths(count: number, now = new Date()): MonthCol[] {
  ...
  for (let i = count - 1; i >= 0; i--) {
    ...
    cols.push({
      key: `${yy}-${pad2(mm)}`,
      label: yy === y ? `${mm}월` : `${String(yy).slice(2)}.${pad2(mm)}`,
      isCurrent: i === 0,
    });
  }
  return cols;
