...
export function PlayPromptModal({ songTitle, onPlay, onCancel }: PlayPromptModalProps) {
  ...
  return (
    ...
    >
      <div
        className="relative w-full max-w-sm max-h-[calc(100dvh-2rem)] overflow-y-auto rounded-2xl border border-purple-500/30 bg-gradient-to-br from-slate-800 to-purple-900/80 p-6 sm:p-8 text-center shadow-2xl animate-in zoom-in-95 duration-300"
      ...
      >
        <button
          ...
          onClick={onCancel}
          aria-label="닫기"
          className="absolute top-2 right-2 p-2 text-gray-400 transition-colors hover:text-white"
        >
          <X className="h-5 w-5" />
        </button>
        ...
        >
          ...
          <PlayCircle
            className="relative mx-auto h-20 w-20 sm:h-24 sm:w-24 text-pink-400 drop-shadow-[0_0_18px_rgba(236,72,153,0.6)] transition-transform duration-300 group-hover:scale-110 group-active:scale-95"
            strokeWidth={1.5}
          />
        </button>
