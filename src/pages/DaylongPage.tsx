...
//   · 헤더: '관리' 배지 + 설정(톱니) → SettingsDialog(제목·잔액 맞추기·조정 내역 삭제·PIN 변경). 전체 거래를 넘긴다.
//     '잔액 조정' 거래는 입출금 내역에 나오지 않고 잔액에만 반영된다(회원에게 노출 안 함). 관리자 조회·삭제는 설정 모달.
...
//   · 잔액 = 모든 거래 합(조정 포함, 시작 잔액 없음). '거래 N건' 은 조정 제외. 상단 '현재 잔액' 카드가 유일한 잔액 큰 숫자. → utils/daylongCalc
...
export default function DaylongPage({ onClose, isAdmin = false }: DaylongPageProps = {}) {
  ...
  // 잔액(모든 거래 합)
  const { balance, countedCount } = useMemo(() => computeBalance(transactions), [transactions]);
  ...
  const balanceNote = describeBalanceNote(countedCount);
